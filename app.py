...
def create_certificate(user_name, topic):
    ...
    <head>
        ...
        <style>
            ...
            h2 {{ font-size: 30px; color: #c0392b; margin: 15px 0; padding-bottom: 10px; border-bottom: 2px solid #ecf0f1; display: inline-block; text-transform: uppercase; }}
    ...
    <body>
        <div class="cert-container">
            ...
            <!-- ଏଠାରେ ୟୁଜର୍ ଙ୍କ ନାମ ଆସିବ -->
            <div class="user-name">{user_name}</div>
            
            <div class="subtitle">for successfully mastering the topic of</div>
            <h2>{topic}</h2>
...
if st.session_state.app_mode == "📚 Study Masterclass" and st.session_state.stage in ["quiz_evaluation", "completion"] and st.session_state.current_session_id:
    ...
    with col_c:
        if st.session_state.certificate is None:
            # ୟୁଜର୍ ଙ୍କ ନାମ ମାଗିବା ପାଇଁ ଇନପୁଟ୍ ବକ୍ସ
            cert_name = st.text_input("Enter your name for Certificate:")
            
            if st.button("🎓 Generate Certificate", use_container_width=True):
                # ଯଦି ୟୁଜର୍ ନାମ ଲେଖି ନାହାଁନ୍ତି, ତେବେ ତାଙ୍କୁ ଆଲର୍ଟ କରିବା
                if cert_name.strip() == "":
                    st.error("⚠️ ଦୟାକରି ସାର୍ଟିଫିକେଟ୍ ପାଇଁ ପ୍ରଥମେ ନିଜର ନାମ ଲେଖନ୍ତୁ!")
                else:
                    with st.spinner("Designing Certificate..."):
                        # ୟୁଜର୍ ଦେଇଥିବା ନାମ (cert_name) କୁ ଫଙ୍କସନ୍ କୁ ପଠାଯାଉଛି
...
if user_input:
    ...
    """
                        ...
                        if any(word in user_input.lower() for word in ["start", "yes", "ok", "ready", "ହଁ", "ଆରମ୍ଭ"]):
                            # ନୂଆ Mega Prompt ଯାହା AI କୁ ୧୦ ଗୁଣା ଲମ୍ବା ଉତ୍ତର ଦେବାକୁ ବାଧ୍ୟ କରିବ
                            prompt = f"""
                            Topic: '{st.session_state.topic}'
                            {persona_instruction}
                            ...
                            Do NOT summarize. Expand on EVERY single point as much as possible (imagine writing a 10-page document).
                            
                            Structure your massive response strictly as follows:
                            1. **Deep Introduction & History:** Core philosophy and why it exists.
                            2. **Exhaustive Breakdown:** Explain every component, rule, and syntax in extreme detail.
                            3. **Real-World Scenarios:** At least 5 detailed real-world examples.
                            4. **Extensive Code Blocks:** Provide multiple complex code examples for every concept (if technical).
                            5. **Advanced Concepts & Best Practices:** Memory management, architecture, and pro tips.
                            6. **Common Pitfalls & Interview Questions:** What mistakes beginners make and how to fix them.
